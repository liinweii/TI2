...
void __fastcall TForm1::UpdateLabelsInfo()
{
	...
	Label1->Caption = "Bits entered: " + IntToStr(actualLength) + " of " +
	...
	Label4->Caption = "Polinom: x^27 + x^8 + x^7 + x + 1";
...
void __fastcall TForm1::AutoGenerateKey()
{
	if (!originalData || originalData->Size == 0)
	{
		Application->MessageBox(L"You need to load file!", L"Error", MB_ICONERROR);
        return;
    }
    ...
    if (actualLength != 27)
	{
		Application->MessageBox(L"Enter all of 27 bits of start state",
    ...
    for (int i = 1; i <= initState.Length(); i++)
    {
        if (initState[i] != '0' && initState[i] != '1')
        {
			Application->MessageBox(L"Register state can consist only of 1 and 0",
				L"Enter error", MB_ICONERROR);
			return;
        }
    }
	...
	if (keyLength <= 0)
	{
		Application->MessageBox(L"File is Empty",
				L"Enter error", MB_ICONERROR);
		return;
    }
    ...
    if (currentKey.Length() > 200)
    {
		...
		memoKey->Lines->Add(L"(First 200 bits are shown of " + IntToStr(currentKey.Length()) + L")");
    }
    else
    {
...
void __fastcall TForm1::UpdateBinaryView(TMemo *memo, TMemoryStream *data)
{
    ...
    {
        memo->Lines->Add("...");
		memo->Lines->Add("(First 500 bytes shown of " + IntToStr(size) + ")");
	}
...
void __fastcall TForm1::ProcessFile(bool encrypt)
{
    ...
    {
		Application->MessageBox(L"Load file", L"Error", MB_ICONERROR);
		return;
    }

    if (currentKey.IsEmpty())
	{
		Application->MessageBox(L"Generate the key", L"Error", MB_ICONERROR);
        return;
    }
    ...
    if (currentKey.Length() != requiredLength)
	{
        ...
        if (currentKey.Length() != requiredLength)
        {
			Application->MessageBox(L"Key Generation Error", L"Error", MB_ICONERROR);
			return;
        }
    }
	...
	String operation = encrypt ? L"Encrypted" : L"Decrypted";
...
void __fastcall TForm1::encryptClick(TObject *Sender)
{
	ProcessFile(true);
	save->Enabled = true;
	Application->MessageBox(L"Encrypted", L"Result", MB_ICONINFORMATION);
}
...
void __fastcall TForm1::decryptClick(TObject *Sender)
{
	ProcessFile(false);
	save->Enabled = true;
	Application->MessageBox(L"Decrypted", L"Result", MB_ICONINFORMATION);
}
...
void __fastcall TForm1::saveClick(TObject *Sender)
{
	...
	{
		Application->MessageBox(L"No data to save",
	...
	if (SaveDialog1->Execute())
	{
		...
		Application->MessageBox(L"Saved", L"Result", MB_ICONINFORMATION);
	}
}
...
void __fastcall TForm1::ViewOriginalClick(TObject *Sender)
{
    if (originalData->Size == 0)
    {
		Application->MessageBox(L"No file", L"Error", MB_ICONERROR);
        return;
    }
    UpdateBinaryView(memoOriginalBinary, originalData);
}
...
void __fastcall TForm1::ViewEncryptedClick(TObject *Sender)
{
    if (encryptedData->Size == 0)
    {
		Application->MessageBox(L"No encrypted data", L"Error", MB_ICONERROR);
		return;
    }
    UpdateBinaryView(memoEncryptedBinary, encryptedData);
}
...
void __fastcall TForm1::edtInitStateKeyPress(TObject *Sender, System::WideChar &Key)
{
	...
	{
		Application->MessageBox(L"You can only enter 0 and 1", L"Enter error", MB_ICONWARNING);
    }
}
